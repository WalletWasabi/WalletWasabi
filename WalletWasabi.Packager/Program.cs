		...
		private static void Main(string[] args)
		{
			...
			if (doPublish)
			{
				...
				foreach (string target in targets)
				{
					...
					{
						FileName = "dotnet",
						Arguments = $"publish --configuration Release --force --output \"{currentBinDistDirectory}\" --self-contained true --runtime \"{target}\" /p:VersionPrefix={versionPrefix} --disable-parallel --no-cache /p:DebugType=none /p:DebugSymbols=false /p:ErrorReport=none /p:DocumentationFile=\"\"",
						WorkingDirectory = guiProjectDirectory
					};
