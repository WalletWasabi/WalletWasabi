...
public static class MacSignTools
{
	...
	public static void Sign()
	{
		...
		Console.WriteLine($"Phase: finding the zip file on'{sourceFolder}' which contains the compiled binaries from Windows.");

		var osxFile = Directory.GetFiles(sourceFolder, "_WasabiToMac-osx-*.zip").Single();
		var osxArm64File = Directory.GetFiles(sourceFolder, "_WasabiToMac-osxarm64-*.zip").Single();

		var versionPrefix = osxFile.Split('-')[^1].TrimEnd(".zip", StringComparison.InvariantCultureIgnoreCase); // Example: "/Users/user/Desktop/Wasabi-unsigned-1.1.10.2.zip".

		(string SourceZipPath, string DestinationDmgFileName)[]? sourceZipAndDestination = new[] {
		...
		foreach (var (sourceZipPath, destinationDmgFileName) in sourceZipAndDestination)
		{
			...
			var usbDmgFilePath = Path.Combine(sourceFolder, destinationDmgFileName);
			...
			File.Move(dmgFilePath, usbDmgFilePath, true);
			DeleteWithChmod(workingDir);
