...
function httpGetTumbler(path: string, returnJson = true) {
    let theUrl: string = "http://localhost:37120/api/v1/tumbler/" + path;
    let xmlHttp: XMLHttpRequest = new XMLHttpRequest();
    xmlHttp.open("GET", theUrl, false); // false for synchronous request
    xmlHttp.send(null);
...
function httpGetTumblerAsync(path: string, callback) {
    let theUrl: string = "http://localhost:37120/api/v1/tumbler/" + path;
    let xmlHttp: XMLHttpRequest = new XMLHttpRequest();
