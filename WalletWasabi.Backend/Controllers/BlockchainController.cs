...
public class BlockchainController : ControllerBase
{
	...
	public async Task<IActionResult> GetFilters([FromQuery, Required] string bestKnownBlockHash, [FromQuery, Required] int count)
	{
		if (count <= 0)
		{
		...
		var (bestHeight, filters, found) = await IndexBuilderService.GetFilterLinesExcludingAsync(knownHash, count);
