...
public class Global
{
	...
	public Global(string dataDir, string configFilePath, Config config)
	{
		...
		WalletFactory walletFactory = new(DataDir, config.Network, BitcoinStore, wasabiSynchronizer, config.ServiceConfiguration, HostedServices.Get<FeeRateEstimationUpdater>(), BlockDownloadService);
