...
public class WasabiApplication
{
	...
	private void BeforeStarting()
	{
		...
		Logger.Log(LogLevel.Info, $"{AppConfig.AppName} started ({InstanceGuid}).", callerFilePath: "", callerLineNumber: -1);
	...
	private void BeforeStopping()
	{
		...
		Logger.Log(LogLevel.Info, $"{AppConfig.AppName} stopped gracefully ({InstanceGuid}).", callerFilePath: "", callerLineNumber: -1);
	}
