...
namespace WalletWasabi.WebClients.Wasabi
{
	public class WasabiClient : TorDisposableBase
	{
		...
		public async Task<(bool backendCompatible, bool clientUpToDate)> CheckUpdatesAsync(CancellationToken cancel)
		{
			...
			var clientUpToDate = Constants.ClientVersion.ToVersion(3) >= versions.ClientVersion.ToVersion(3); // If the client version locally is greater than or equal to the backend's reported client version, then good.
