					...
					while (IsRunning)
					{
						try
						{
							...
							var (currentHeight, currentHeader) = lastIndexFilter is not null
								? (lastIndexFilter.Header.Height, lastIndexFilter.Header.Header)
							...
							BlockFilter blockFilter = await _rpcClient.GetBlockFilterAsync(blockHash).ConfigureAwait(false);
