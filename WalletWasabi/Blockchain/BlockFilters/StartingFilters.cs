...
{
	public static FilterModel GetStartingFilter(Network network, RpcPubkeyType[] scriptTypes)
	{
		var startingHeader = SmartHeader.GetStartingHeader(network, scriptTypes);
		...
		{
			GolombRiceFilter filter = Index.CreateDummyEmptyFilter(startingHeader.BlockHash);
