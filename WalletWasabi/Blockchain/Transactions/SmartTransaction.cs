...
public class SmartTransaction : IEquatable<SmartTransaction>
{
	...
	public bool IsRbfable(KeyManager keyManager) =>
		...
		&& GetForeignOutputs(keyManager).Any() // [Nonsense] There must be at least some foreign output. Otherwise it's self spend, which does not make sense to be sped up, because there, no economic transaction is taking place.
		&& WalletOutputs.All(x => !x.IsSpent()); // [Danger] All the outputs we know of should not be spent, otherwise we shouldn't do RBF.
