...
public class TransactionProcessor
{
	...
	private ProcessedResult ProcessNoLock(SmartTransaction tx)
	{
		...
		var spScripts = tx.TweakData != null
			? KeyManager.GetSilentPaymentScanData().SelectMany(x => SilentPayment
						.ExtractSilentPaymentScriptPubKeys([x.Address], tx.TweakData, tx.Transaction, x.ScanSecret)
						.SelectMany(y => y.Value)).ToArray()
			: [];
