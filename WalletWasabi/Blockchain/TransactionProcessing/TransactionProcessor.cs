...
public class TransactionProcessor
{
	...
	private ProcessedResult ProcessNoLock(SmartTransaction tx)
	{
		...
		{
			scanKey = KeyManager.GetKeys(x => x.FullKeyPath.GetAccountKeyPath() == KeyManager.GetAccountKeyPath(Network.Main, KeyPurpose.Scan)).First();
			var spendKey = KeyManager.GetKeys(x => x.FullKeyPath.GetAccountKeyPath() == KeyManager.GetAccountKeyPath(Network.Main, KeyPurpose.Spend)).First();
			var address = new SilentPaymentAddress(0, ECPubKey.Create(scanKey.PubKey.ToBytes()), ECPubKey.Create(spendKey.PubKey.ToBytes()));
			var extKey = KeyManager.GetMasterExtKey("password");
			var scanSecret = extKey.Derive(scanKey.FullKeyPath);
			var s = SilentPayment.ExtractSilentPaymentScriptPubKeys([address], tx.TweakData, tx.Transaction, ECPrivKey.Create(scanSecret.PrivateKey.ToBytes()));
			spScripts.AddRange(s);
		}
