...
try {
  ...
  if ($skipExtractingBrowserArchives) {
    ...
    Write-Output "# Extract Tor Browser for linux (arm64)."
    & $sevenZip "x" "$linuxCompressedTarball_arm64" "-y"
    & $sevenZip "x" "$linuxTarball_arm64" "-oTorBrowser/linux-arm64"

    Write-Output "# Extract Tor Browser for linux (x64)."
    & $sevenZip "x" "$linuxCompressedTarball_x86_64" "-y"
    & $sevenZip "x" "$linuxTarball_x86_64" "-oTorBrowser/linux-x64"
  }
  ...
  if ($skipExtractingTorBinaries) {
    ...
    mkdir -p "Tor/linux-arm64/"
    Copy-item -Recurse -Force -Path "TorBrowser/linux-arm64/tor-browser/Browser/TorBrowser/Tor/*" -Destination "Tor/linux-arm64/" -Exclude "PluggableTransports"

    Write-Output "# Extract Tor binary for linux (x64)."
    mkdir -p "Tor/linux-x64/"
    Copy-item -Recurse -Force -Path "TorBrowser/linux-x64/tor-browser/Browser/TorBrowser/Tor/*" -Destination "Tor/linux-x64/" -Exclude "PluggableTransports"
  }
  ...
  if ($skipReplacingGeoIpFiles) {
    ...
    Copy-item -Force -Path "TorBrowser/linux-x64/tor-browser/Browser/TorBrowser/Data/Tor/geoip*" -Destination "$destination/"
  }
