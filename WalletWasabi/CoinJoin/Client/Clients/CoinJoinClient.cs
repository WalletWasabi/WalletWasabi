...
namespace WalletWasabi.CoinJoin.Client.Clients
{
	public class CoinJoinClient
	{
		...
		private async Task RegisterOutputAsync(ClientRound ongoingRound)
		{
			...
			{
				IHttpClient httpClient = Synchronizer.HttpClientFactory.NewBackendHttpClient(Mode.NewIdentityPerRequest);
		...
		private async Task<AliceClientBase> CreateAliceClientAsync(long roundId, List<OutPoint> registrableCoins, (HdPubKey change, IEnumerable<HdPubKey> actives) outputAddresses)
		{
			...
			IHttpClient satoshiHttpClient = factory.NewBackendHttpClient(Mode.NewIdentityPerRequest);
			...
			IHttpClient httpClient = Synchronizer.HttpClientFactory.NewHttpClient(CcjHostUriAction, Mode.NewIdentityPerRequest);
