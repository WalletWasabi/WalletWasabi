...
public class CoordinatorRound
{
	...
	internal async Task TryOptimizeFeesAsync(Transaction transaction, IEnumerable<Coin> spentCoins)
	{
		try
		{
			...
			FeeRate optimalFeeRate = (await RpcClient.EstimateSmartFeeAsync(AdjustedConfirmationTarget, simulateIfRegTest: true).ConfigureAwait(false)).FeeRate;
	...
	public static async Task<(Money feePerInputs, Money feePerOutputs)> CalculateFeesAsync(IRPCClient rpc, int confirmationTarget)
	{
		...
		try
		{
			var feeRate = (await rpc.EstimateSmartFeeAsync(confirmationTarget, simulateIfRegTest: true).ConfigureAwait(false)).FeeRate;
