...
public static partial class Decode
{
	private static readonly Decoder<Scalar> Scalar =
		Hexadecimal.AndThen(bytes => Succeed(new Scalar(bytes)));

	private static readonly Decoder<GroupElement> GroupElement =
		Hexadecimal.AndThen(bytes => Succeed(global::WabiSabi.Crypto.Groups.GroupElement.FromBytes(bytes)));
	...
	private static readonly Decoder<GroupElementVector> GroupElementVector =
		Array(GroupElement).AndThen(a => Succeed(CreateInstance<GroupElementVector>(a)));

	private static readonly Decoder<ScalarVector> ScalarVector =
		Array(Scalar).AndThen(a => Succeed(CreateInstance<ScalarVector>(a.Cast<object>().ToArray())));
