...
public delegate JsonNode Encoder<T>(T value);
...
public static partial class Decode
{
	...
	public static Decoder<short> Int16 =>
		value => Integral("a short", short.TryParse, short.MinValue, short.MaxValue, Convert.ToInt16, value);
	...
	public static Decoder<Dictionary<string, T>> Dictionary<T>(Decoder<T> decoder) =>
		value =>
		{
			...
			return Result<Dictionary<string,T>, string>.Ok(value.EnumerateObject().ToDictionary(x => x.Name, x => decoder(x.Value).Value));
		};
	...
	public static Decoder<T> Field<T>(string fieldName, Decoder<T> decoder) =>
		value =>
		{
			if (value.ValueKind != JsonValueKind.Object)
			{
				return Result<T,string>.Fail("It is not an object");
			}
	...
	public class Getters(JsonElement value)
	{
		...
		public T Required<T>(string fileName, Decoder<T> decoder) =>
			Field(fileName, decoder)(value)
				.Match( v => v, e => { Errors.Add(e); return default!; });

		public T? Optional<T>(string fileName, Decoder<T> decoder) =>
			Field(fileName, decoder)(value).Match(v => v, _ => (T?)(object?)null);
	}
