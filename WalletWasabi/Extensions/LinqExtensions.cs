...
public static class LinqExtensions
{
	...
	public static T? RandomElement<T>(this IEnumerable<T> source, WasabiRandom? random = null)
	{
		T? current = default;
		int count = 0;
		...
		{
			count++;
			var rnd = random is null ? Random.Shared.Next(count) : random.GetInt(0, count);
			if (rnd == 0)
			{
				current = element;
			}
	...
	public static IList<T> Shuffle<T>(this IList<T> list, WasabiRandom? random = null)
	{
		int n = list.Count;
		while (n > 1)
		{
			n--;
			var k = random is null ? Random.Shared.Next(n + 1) : random.GetInt(0, n + 1);
			T value = list[k];
			list[k] = list[n];
			list[n] = value;
		}
	...
	public static IList<T> ToShuffled<T>(this IEnumerable<T> list, WasabiRandom? random = null)
	{
