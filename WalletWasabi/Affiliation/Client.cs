...
public class AffiliateServerHttpApiClient : IAffiliateServerApi
{
	...
	public Task<PaymentDataResponse> GetPaymentData(PaymentDataRequest request, CancellationToken cancellationToken) =>
		SendAsync<PaymentDataRequest, PaymentDataResponse>(RemoteAction.GetPaymentData, request, TimeSpan.FromSeconds(10), 6, cancellationToken);
	...
	private static string GetUriEndPoint(RemoteAction action) =>
		action switch
		{
			RemoteAction.GetPaymentData => "get_coinjoin_request",
