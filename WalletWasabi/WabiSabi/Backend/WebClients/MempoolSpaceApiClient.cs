...
public class MempoolSpaceApiClient : BaseApiClient, ITxPropagationVerifier
{
	...
	public MempoolSpaceApiClient(Network network, HttpClient httpClient) : base(SetBaseAddress(network, httpClient))
	{
		Network = network;
	}
	...
	public async Task<bool?> GetTransactionStatusAsync(uint256 txid, CancellationToken cancel)
	{
		...
		var response = await SendRequestAsync(request, "mempoolspace-request", cancel).ConfigureAwait(false);
