...
public partial class Arena : PeriodicRunner
{
		...
		foreach (var round in Rounds.Where(x => x.Phase == Phase.OutputRegistration).ToArray())
		{
			try
			{
				...
				if (allReady || phaseExpired)
				{
					...
					var highestFeeRateTask = async () => (await Rpc.EstimateSmartFeeAsync(2, simulateIfRegTest: true, cancellationToken).ConfigureAwait(false)).FeeRate;
		...
		foreach (var round in Rounds.Where(x => x.Phase == Phase.TransactionSigning).ToArray())
		{
			...
			try
			{
				if (state.IsFullySigned)
				{
					...
					try
					{
						FeeRate targetFeeRate = (await Rpc.EstimateSmartFeeAsync((int)Config.ConfirmationTarget, simulateIfRegTest: true, cancellationToken).ConfigureAwait(false)).FeeRate;
	...
	{
		var feeRate = (await Rpc.EstimateSmartFeeAsync((int)Config.ConfirmationTarget, simulateIfRegTest: true, cancellationToken).ConfigureAwait(false)).FeeRate;
	...
	private async Task CreateRoundsAsync(CancellationToken cancellationToken)
	{
		...
		if (Config.WW200CompatibleLoadBalancing)
		{
			...
			{
				feeRate = (await Rpc.EstimateSmartFeeAsync((int)Config.ConfirmationTarget, simulateIfRegTest: true, cancellationToken).ConfigureAwait(false)).FeeRate;
