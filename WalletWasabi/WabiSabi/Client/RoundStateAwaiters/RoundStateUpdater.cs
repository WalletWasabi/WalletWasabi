...
{
	public record UpdateMessage : RoundUpdateMessage;
...
public class RoundStateProvider(MailboxProcessor<RoundUpdateMessage> roundStateUpdater)
{
		...
		CancellationToken cancellationToken)
	{
		...
		return await awaiter.WaitAsync(cancellationToken).ConfigureAwait(false);
	}
...
public record RoundsState(Dictionary<uint256, RoundState> Rounds, ImmutableList<RoundStateAwaiter> Awaiters);
...
public static class RoundStateUpdater
{
		...
		CancellationToken cancellationToken)
	{
		switch (msg)
		{
			case RoundUpdateMessage.UpdateMessage _:
				state = await UpdateRoundsStateAsync(state, arenaRequestHandler, cancellationToken).ConfigureAwait(false);
				break;
	...
	private static async Task<RoundsState> UpdateRoundsStateAsync(
		RoundsState state,
		IWabiSabiApiRequestHandler arenaRequestHandler,
		CancellationToken cancellationToken)
	{
		...
		return new RoundsState(Rounds: finalRoundStates, Awaiters: state.Awaiters.RemoveRange(completedAwaiters));
	}
}
