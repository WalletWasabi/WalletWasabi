...
public class CoinPrison : IDisposable
{
	...
	///	With this, the coin will retry to participate in a CJ in every 2 days and see if the coin is still banned or not according to the backend.
	/// </summary>
	...
	private static DateTimeOffset ReduceBanningTimeIfNeeded(DateTimeOffset bannedUntil)
	{
		...
		{
			return currentDate.AddDays(MaxDaysToTrustLocalPrison);
		}
