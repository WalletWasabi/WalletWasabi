...
public class CoinJoinManager : BackgroundService
{
	...
	public ImmutableDictionary<Guid, ImmutableList<SmartCoin>> CoinsInCriticalPhase { get; set; } = ImmutableDictionary<Guid, ImmutableList<SmartCoin>>.Empty;
	...
	/// </summary>
	private ConcurrentDictionary<Guid, UiBlockedStateHolder> WalletsBlockedByUi { get; } = new();
	...
	private ImmutableDictionary<Guid, CoinJoinClientStateHolder> CoinJoinClientStates { get; set; } = ImmutableDictionary<Guid, CoinJoinClientStateHolder>.Empty;
	...
	public CoinJoinClientState GetCoinjoinClientState(Guid walletId)
	{
	...
	private async Task MonitorAndHandleCoinJoinsAsync(CancellationToken stoppingToken)
	{
		...
		var trackedCoinJoins = new ConcurrentDictionary<Guid, CoinJoinTracker>();
	...
	private async Task HandleCoinJoinCommandsAsync(ConcurrentDictionary<Guid, CoinJoinTracker> trackedCoinJoins, ConcurrentDictionary<IWallet, TrackedAutoStart> trackedAutoStarts, CancellationToken stoppingToken)
	{
		...
		async void StartCoinJoinCommand(StartCoinJoinCommand startCommand)
		{
			...
			if (trackedCoinJoins.TryGetValue(walletToStart.Id, out var tracker))
			{
			...
			{
				if (WalletsBlockedByUi.ContainsKey(walletToStart.Id))
				{
			...
			if (!trackedCoinJoins.TryAdd(walletToStart.Id, coinJoinTracker))
			{
		...
		void StopCoinJoinCommand(StopCoinJoinCommand stopCommand)
		{
			...
			if (trackedCoinJoins.TryGetValue(walletToStop.Id, out var coinJoinTrackerToStop))
			{
	...
	private void ScheduleRestartAutomatically(IWallet walletToStart, ConcurrentDictionary<IWallet, TrackedAutoStart> trackedAutoStarts, bool stopWhenAllMixed, bool overridePlebStop, IWallet outputWallet, CancellationToken stoppingToken)
	{
		...
		{
			if (stopWhenAllMixed == trackedAutoStart.StopWhenAllMixed && overridePlebStop == trackedAutoStart.OverridePlebStop && outputWallet.Id == trackedAutoStart.OutputWallet.Id)
			{
...
#pragma warning restore CA2000 // Dispose objects before losing scope
	...
	private async Task MonitorAndHandlingCoinJoinFinalizationAsync(ConcurrentDictionary<Guid, CoinJoinTracker> trackedCoinJoins, ConcurrentDictionary<IWallet, TrackedAutoStart> trackedAutoStarts, CancellationToken stoppingToken)
	{
		while (!stoppingToken.IsCancellationRequested)
		{
	...
	private ImmutableDictionary<Guid, ImmutableList<SmartCoin>> GetCoinsInCriticalPhase(IEnumerable<IWallet> wallets, ConcurrentDictionary<Guid, CoinJoinTracker> trackedCoinJoins)
	{
		var coinsUsedInCoinjoins = ImmutableDictionary.CreateBuilder<Guid, ImmutableList<SmartCoin>>();

		foreach (var wallet in wallets)
		{
			ImmutableList<SmartCoin> coinsInCoinjoin = ImmutableList<SmartCoin>.Empty;

			if (trackedCoinJoins.TryGetValue(wallet.Id, out var coinJoinTracker) && !coinJoinTracker.IsCompleted)
			{
				coinsInCoinjoin = coinJoinTracker.CoinsInCriticalPhase;
			}

			coinsUsedInCoinjoins.Add(wallet.Id, coinsInCoinjoin);
		}

		return coinsUsedInCoinjoins.ToImmutable();
	}

	private static ImmutableDictionary<Guid, CoinJoinClientStateHolder> GetCoinJoinClientStates(IEnumerable<IWallet> wallets, ConcurrentDictionary<Guid, CoinJoinTracker> trackedCoinJoins, ConcurrentDictionary<IWallet, TrackedAutoStart> trackedAutoStarts)
	{
		var coinJoinClientStates = ImmutableDictionary.CreateBuilder<Guid, CoinJoinClientStateHolder>();
		foreach (var wallet in wallets)
		{
			...
			coinJoinClientStates.Add(wallet.Id, state);
		}

		return coinJoinClientStates.ToImmutable();
	}

	private async Task HandleCoinJoinFinalizationAsync(CoinJoinTracker finishedCoinJoin, ConcurrentDictionary<Guid, CoinJoinTracker> trackedCoinJoins, ConcurrentDictionary<IWallet, TrackedAutoStart> trackedAutoStarts, CancellationToken cancellationToken)
	{
		...
		if (!trackedCoinJoins.TryRemove(wallet.Id, out _))
		{
	...
	private async Task<ImmutableDictionary<Guid, IWallet>> GetMixableWalletsAsync() =>
	...
	public void WalletEnteredSendWorkflow(Guid walletId)
	{
	...
	{
		if (!WalletsBlockedByUi.TryRemove(wallet.Id, out var stateHolder))
		{
	...
	{
		if (!WalletsBlockedByUi.ContainsKey(wallet.Id))
		{
			Logger.LogDebug("Wallet tried to enter sending but it was not in the send workflow.");
			return;
		}

		if (!CoinJoinClientStates.TryGetValue(wallet.Id, out var stateHolder))
		{
		...
		{
			WalletsBlockedByUi[wallet.Id] = new UiBlockedStateHolder(NeedRestart: true, stateHolder.StopWhenAllMixed, stateHolder.OverridePlebStop, stateHolder.OutputWallet);
		}
	...
	public async Task SignalToStopCoinjoinsAsync()
	{
		...
		{
			if (CoinJoinClientStates.TryGetValue(wallet.Id, out var stateHolder) && stateHolder.CoinJoinClientState is not CoinJoinClientState.Idle)
			{
				if (!WalletsBlockedByUi.TryAdd(wallet.Id, new UiBlockedStateHolder(true, stateHolder.StopWhenAllMixed, stateHolder.OverridePlebStop, stateHolder.OutputWallet)))
				{
					WalletsBlockedByUi[wallet.Id] = new UiBlockedStateHolder(true, stateHolder.StopWhenAllMixed, stateHolder.OverridePlebStop, stateHolder.OutputWallet);
				}
	...
	public async Task RestartAbortedCoinjoinsAsync()
	{
		foreach (var wallet in await WalletProvider.GetWalletsAsync().ConfigureAwait(false))
		{
			if (WalletsBlockedByUi.TryRemove(wallet.Id, out var stateHolder) && stateHolder.NeedRestart)
			{
