...
public class CoinJoinManager : BackgroundService
{
	...
	private ImmutableDictionary<string, Wallet> GetMixableWallets() =>
		WalletManager.GetWallets()
			...
			.Where(x => (x.KeyManager.AutoCoinJoin && (DateTimeOffset.UtcNow - x.StartedTime) > AutoCoinJoinDelayAfterWalletLoaded) || x.AllowManualCoinJoin)     // configured to be mixed automatically or manually
