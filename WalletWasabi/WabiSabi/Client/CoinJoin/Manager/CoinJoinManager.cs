...
public class CoinJoinManager : BackgroundService
{
	...
	public async Task RequestCoinJoinStartAsync(Wallet wallet, Wallet outputWallet, bool isAutoCoinJoinEnabled, bool overridePlebStop)
	{
		...
		var command = new StartCoinJoinCommand(wallet, outputWallet, isAutoCoinJoinEnabled, overridePlebStop);
	...
	private async void HandleStartCoinJoinCommandAsync(StartCoinJoinCommand startCommand, CoinJoinTrackerFactory coinJoinTrackerFactory)
	{
		...
		{
			if (startCommand.IsAutoCoinJoinEnabled != tracker.IsAutoCoinJoinEnabled)
			{
				tracker.IsAutoCoinJoinEnabled = startCommand.IsAutoCoinJoinEnabled;
				Logger.LogDebug(FormatLog($"Cannot start coinjoin, because it is already running - but updated the value of {nameof(startCommand.IsAutoCoinJoinEnabled)} to {startCommand.IsAutoCoinJoinEnabled}.", walletToStart));
			}
			else
			{
			...
			if (!walletToStart.BatchedPayments.AreTherePendingPayments)
			{
				...
				if (coinCandidates.All(x => x.IsPrivate(walletToStart.AnonScoreTarget)))
				{
					throw new CoinJoinClientException(
						...
						$"All coin candidates are already private and {nameof(startCommand.IsAutoCoinJoinEnabled)} was {startCommand.IsAutoCoinJoinEnabled}");
				}
			}
		...
		var coinJoinTracker = await coinJoinTrackerFactory.CreateAndStartAsync(walletToStart, startCommand.OutputWallet, SanityChecksAndGetCoinCandidatesFunc, startCommand.IsAutoCoinJoinEnabled, startCommand.OverridePlebStop).ConfigureAwait(false);
		...
		Logger.LogDebug(FormatLog($"{nameof(startCommand.IsAutoCoinJoinEnabled)}:'{startCommand.IsAutoCoinJoinEnabled}' {nameof(startCommand.OverridePlebStop)}:'{startCommand.OverridePlebStop}'.", walletToStart));
	...
	private void ScheduleRestartAutomatically(Wallet walletToStart, ConcurrentDictionary<WalletId, TrackedAutoStart> trackedAutoStarts, bool isAutoCoinJoinEnabled, bool overridePlebStop, Wallet outputWallet, CancellationToken stoppingToken)
	{
		var skipDelay = false;
		if (trackedAutoStarts.TryGetValue(walletToStart.WalletId, out var trackedAutoStart))
		{
			if (isAutoCoinJoinEnabled == trackedAutoStart.IsAutoCoinJoinEnabled && overridePlebStop == trackedAutoStart.OverridePlebStop && outputWallet.WalletId == trackedAutoStart.OutputWallet.WalletId)
			{
...
#pragma warning restore CA2000 // Dispose objects before losing scope
		...
		var restartTask = new Task(
			async () =>
			{
				...
				{
					await RequestCoinJoinStartAsync(walletToStart, outputWallet, isAutoCoinJoinEnabled, overridePlebStop).ConfigureAwait(false);
				}
				else
				{
		...
		if (trackedAutoStarts.TryAdd(walletToStart.WalletId, new TrackedAutoStart(restartTask, isAutoCoinJoinEnabled, overridePlebStop, outputWallet, linkedCts)))
		{
			restartTask.Start();
		}
		else
	...
	private ImmutableDictionary<WalletId, CoinJoinClientStateHolder> GetCoinJoinClientStates(IEnumerable<Wallet> wallets)
	{
		...
		foreach (var wallet in wallets)
		{
			...
			if (_state.TrackedCoinJoins.TryGetValue(wallet.WalletId, out var coinJoinTracker) && !coinJoinTracker.IsCompleted)
			{
				...
				clientState = new(trackerState, coinJoinTracker.IsAutoCoinJoinEnabled, coinJoinTracker.OverridePlebStop, coinJoinTracker.OutputWallet);
			}
			else if (_state.TrackedAutoStarts.TryGetValue(wallet.WalletId, out var autoStartTracker))
			{
				clientState = new(CoinJoinClientState.InSchedule, autoStartTracker.IsAutoCoinJoinEnabled, autoStartTracker.OverridePlebStop, autoStartTracker.OutputWallet);
			}
	...
	private async Task HandleCoinJoinFinalizationAsync(CoinJoinTracker finishedCoinJoin, CancellationToken cancellationToken)
	{
		...
		else if (wallet.IsWalletPrivate())
		{
			...
			if (!finishedCoinJoin.IsAutoCoinJoinEnabled)
			{
				// In auto CJ mode we never stop trying.
				ScheduleRestartAutomatically(wallet, _state.TrackedAutoStarts, finishedCoinJoin.IsAutoCoinJoinEnabled, finishedCoinJoin.OverridePlebStop, finishedCoinJoin.OutputWallet, cancellationToken);
			}
			else
			{
	...
	public void WalletEnteredSendWorkflow(WalletId walletId)
	{
		...
		{
			_state.WalletsBlockedByUi.TryAdd(walletId, new UiBlockedStateHolder(NeedRestart: false, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop, stateHolder.OutputWallet));
		}
	}
	...
	public void WalletLeftSendWorkflow(Wallet wallet)
	{
		...
		{
			Task.Run(async () => await RequestCoinJoinStartAsync(wallet, stateHolder.OutputWallet, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop).ConfigureAwait(false));
		}
	}
	...
	public async Task WalletEnteredSendingAsync(Wallet wallet)
	{
		...
		{
			_state.WalletsBlockedByUi[wallet.WalletId] = new UiBlockedStateHolder(NeedRestart: true, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop, stateHolder.OutputWallet);
		}
	...
	public async Task SignalToStopCoinjoinsAsync()
	{
		foreach (var wallet in await _getWalletsAsync().ConfigureAwait(false))
		{
			...
			{
				if (!_state.WalletsBlockedByUi.TryAdd(wallet.WalletId, new UiBlockedStateHolder(true, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop, stateHolder.OutputWallet)))
				{
					_state.WalletsBlockedByUi[wallet.WalletId] = new UiBlockedStateHolder(true, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop, stateHolder.OutputWallet);
				}
				await RequestCoinJoinStopAsync(wallet).ConfigureAwait(false);
			}
		}
	...
	public async Task RestartAbortedCoinjoinsAsync()
	{
		foreach (var wallet in await _getWalletsAsync().ConfigureAwait(false))
		{
			...
			{
				await RequestCoinJoinStartAsync(wallet, stateHolder.OutputWallet, stateHolder.IsAutoCoinJoinEnabled, stateHolder.OverridePlebStop).ConfigureAwait(false);
			}
		}
	}
	...
	private record StartCoinJoinCommand(Wallet Wallet, Wallet OutputWallet, bool IsAutoCoinJoinEnabled, bool OverridePlebStop) : CoinJoinCommand(Wallet);
	private record StopCoinJoinCommand(Wallet Wallet) : CoinJoinCommand(Wallet);

	private record TrackedAutoStart(Task Task, bool IsAutoCoinJoinEnabled, bool OverridePlebStop, Wallet OutputWallet, CancellationTokenSource CancellationTokenSource);
	private record CoinJoinClientStateHolder(CoinJoinClientState CoinJoinClientState, bool IsAutoCoinJoinEnabled, bool OverridePlebStop, Wallet OutputWallet);
	private record UiBlockedStateHolder(bool NeedRestart, bool IsAutoCoinJoinEnabled, bool OverridePlebStop, Wallet OutputWallet);
	...
	/// The other 2 is only needed not to loose the IsAutoCoinJoinEnabled and OverridePlebStop configuration.
