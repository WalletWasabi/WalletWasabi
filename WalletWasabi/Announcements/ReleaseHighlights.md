## Release Highlights
#### 💰 Sub-1 sat/vbyte transaction fee support
...
Signet is another test network that allows a set of signers to create blocks. This reduces the unpredictable behavior so developers can work with a stable environment. You must specify an indexer in the Signet config file before launching the client on that network.
...
### 🔎 Bug fixes
BIP352 silent payment transactions can be sent again. Hundreds of build warnings were fixed. Software verification and security patches are included thanks to Ledger’s Donjon team.
