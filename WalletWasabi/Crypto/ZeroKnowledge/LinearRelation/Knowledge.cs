...
namespace WalletWasabi.Crypto.ZeroKnowledge.LinearRelation
{
	internal class Knowledge
	{
		...
		{
			_ = Guard.True(nameof(witness), witness.Count == statement.Equations.First().Generators.Count, $"{nameof(witness)} size does not match {nameof(statement)}.{nameof(statement.Equations)}");
