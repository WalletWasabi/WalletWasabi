...
def report(actual_directory: Path, output: Path, baseline: Path | None = None, *, allow_unreviewed: bool = False,
    ...
    for frame in frames:
        ...
        if expected is not None and expected.is_file() and name in approvals:
    ...
    summary = {
        "engine": "avalonia-headless-skia", "total": len(results), "changed": changed,
        "unreviewed": unreviewed, "status": "different" if changed else "unreviewed" if unreviewed else "passed",
        "gatePassed": changed == 0 and (unreviewed == 0 or allow_unreviewed),
    ...
    for item in results:
        ...
        reviewed = item["status"] not in {"unreviewed", "missing-baseline", "unreviewed-baseline"}
...
h1{font-size:32px;margin-bottom:8px}h2{font-size:17px;overflow-wrap:anywhere}p{color:#adbbb3}article{border:1px solid #34453b;border-radius:16px;padding:20px;margin:24px 0}
...
[data-status="different"],[data-status="dimension-mismatch"]{border-color:#c58372}.notice{border-left:3px solid #ddbd74;padding-left:16px}
...
ROWS
<script>function filter(s){document.querySelectorAll('article').forEach(a=>a.hidden=s!=='all'&&(s==='different'?!['different','dimension-mismatch'].includes(a.dataset.status):!['unreviewed','missing-baseline','unreviewed-baseline'].includes(a.dataset.status)))}</script></html>'''
    description = f"{len(results)} frames · {changed} changed · {unreviewed} unreviewed. Status: {summary['status']}."
    (output / "index.html").write_text(page.replace("SUMMARY", html.escape(description)).replace("ROWS", "\n".join(rows)), encoding="utf-8")
    return summary


def approve(actual_directory: Path, baseline: Path, *, reviewed: bool, replace: bool = False) -> int:
    if not reviewed:
        raise ValueError("Review the native captures before enrolling them; --reviewed is required")
    frames = collect_frames(actual_directory)
    conflicts = [frame["image"] for frame in frames if (baseline / frame["image"]).exists()]
    ...
    for frame in frames:
        ...
        previous[frame["image"]] = frame
    (baseline / "approval.json").write_text(json.dumps({"reviewed": True, "frames": [previous[name] for name in sorted(previous)]}, indent=2) + "\n", encoding="utf-8")
    return len(frames)
...
def main() -> int:
    ...
    try:
        if args.command == "approve":
            print(f"Enrolled {approve(args.actual, args.baseline, reviewed=args.reviewed, replace=args.replace)} reviewed captures")
            return 0
