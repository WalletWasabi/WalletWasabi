#!/usr/bin/env bash
#
# Downloads, extracts and upgrades Tor Browser binaries for Wasabi Wallet
#
# Requirements:
...
#   - run from the root of the Wasabi repository (where win-x64, osx64, etc. folders exist)
#
# Usage:
#   ./upgrade-tor.sh 15.0.4
#   ./upgrade-tor.sh 15.0.4 --skip-download
#   ./upgrade-tor.sh 15.0.4 --skip-download --debug
#
...
basedir=$(dirname "$0")
BINARIES_DIR=$(realpath "${basedir}/../../WalletWasabi/Microservices/Binaries")
...
if [[ ! -d "linux-x64" || ! -d "osx64" || ! -d "win-x64" ]]; then
    error "Expected linux-x64, osx64, win-x64, etc. to be present in the folder 'WalletWasabi/Microservices/Binaries'"
fi
...
popd >/dev/null # Working directory is 'WalletWasabi/Microservices/Binaries' again.
...
if [[ "$SKIP_REPLACE_GEOIP" != true ]]; then
    ...
    geoip_target="../../Tor/Geoip"   # Adjust if your folder structure is different.
...
section "Marking Tor and HWI binaries as executable (git)"
...
# Also HWI if present in your repo
