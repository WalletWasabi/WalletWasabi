...
public class UiConfig : ConfigBase
{
	...
	public static UiConfig LoadFile(string filePath)
	{
		try
		{
			...
			var decoder = Decode.FromStream(UiConfigDecode.UiConfig(filePath));
		...
		catch (Exception ex)
		{
			...
			File.WriteAllTextAsync(filePath, UiConfigEncode.UiConfig(config).ToJsonString());
	...
	protected override string EncodeAsJson() =>
		UiConfigEncode.UiConfig(this).ToJsonString(new JsonSerializerOptions
		{
			WriteIndented = true
		});
}
