...
public class Global
{
	...
	private async Task StartTorProcessManagerAsync(CancellationToken cancel)
	{
		if (Config.UseTor && Network != Network.RegTest)
		{
			...
			HostedServices.Register<TorMonitor>(() => new TorMonitor(period: TimeSpan.FromSeconds(3), Config.GetFallbackBackendUri(), TorManager, BackendHttpClientFactory), nameof(TorMonitor));
		}
	}
