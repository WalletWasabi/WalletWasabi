...
public class Global
{
	...
	private async Task StartTorProcessManagerAsync(CancellationToken cancellationToken)
	{
		if (Config.UseTor && Network != Network.RegTest)
		{
			...
			HostedServices.Register<TorMonitor>(() => new TorMonitor(period: TimeSpan.FromSeconds(3), Config.GetCurrentBackendUri(), TorManager, HttpClientFactory), nameof(TorMonitor));
		}
	}
