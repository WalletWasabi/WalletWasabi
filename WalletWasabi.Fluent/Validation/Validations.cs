...
public class Validations : ReactiveObject, IRegisterValidationMethod, IValidations
{
	...
	public bool AnyInfo => ErrorsByPropertyName.Any(x => x.Value.Any(error => error.Severity == ErrorSeverity.Info));

	IEnumerable<string> IValidations.Info => ErrorsByPropertyName.Values.SelectMany(x => x.Where(error => error.Severity == ErrorSeverity.Info).Select(error => error.Message));
	...
	private void OnErrorsChanged(string propertyName, List<ErrorSeverity> categoriesToNotify)
	{
		...
		{
			ErrorSeverity.Info => nameof(AnyInfo),
