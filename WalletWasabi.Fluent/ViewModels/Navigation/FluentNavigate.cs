...
public partial class FluentNavigate
{
	...
	public FluentDialog<bool> CoinJoinAuthDialog(WalletCoinjoinModel walletCoinjoinModel, WalletType walletType, int maxRounds, decimal maxMiningFeeRate, NavigationTarget navigationTarget = NavigationTarget.CompactDialogScreen, NavigationMode navigationMode = NavigationMode.Normal)
	{
		var dialog = new CoinJoinAuthDialogViewModel(UiContext, walletCoinjoinModel, walletType, maxRounds, maxMiningFeeRate);
		var target = UiContext.Navigate(navigationTarget);
		target.To(dialog, navigationMode);
