...
public partial class RecoverWalletViewModel : RoutableViewModel
{
	...
	private async Task OnNextAsync(WalletCreationOptions.RecoverWallet options)
	{
		...
		var password = await Navigate().To().CreatePasswordDialog("Add Passphrase", "If you used a passphrase when you created the wallet type it below, otherwise leave this empty.").GetResultAsync();
