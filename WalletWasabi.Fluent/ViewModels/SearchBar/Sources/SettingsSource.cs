...
public class SettingsSource : ISearchItemSource
{
	...
	private static IEnumerable<ISearchItem> GetSettingsItems()
	{
		return new ISearchItem[]
		{
			...
			new NonActionableSearchItem(new Setting<Config, bool>(Services.Config, b => b.UseTor, true), "Use Tor", "Settings", new List<string>(), "nav_settings_regular") { IsDefault = false },
			new NonActionableSearchItem(new Setting<Config, bool>(Services.Config, b => b.TerminateTorOnExit, true), "Terminate Tor on exit", "Settings", new List<string>(), "nav_settings_regular") { IsDefault = false },
