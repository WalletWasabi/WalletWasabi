...
public partial class PrivacyControlViewModel : DialogViewModelBase<IEnumerable<SmartCoin>>
{
	...
	private async Task InitializeLabelsAsync()
	{
		...
		await LabelSelection.ResetAsync(_wallet.Coins.GetPockets(privateThreshold).Select(x => new Pocket(x)).ToArray(), coinsToExclude: cjManager.CoinsInCriticalPhase[_wallet.WalletName].ToList());
