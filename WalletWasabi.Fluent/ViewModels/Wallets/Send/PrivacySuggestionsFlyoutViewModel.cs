		...
		using (await _asyncLock.LockAsync(CancellationToken.None))
		{
			try
			{
				...
				if (hasChange && !info.IsFixedAmount && !info.IsPayJoin)
				{
					...
					ImmutableArray<SmartCoin> coinsToUse = spentCoins.Any(coinsToExclude.Contains) ? usedPockets.SelectMany(x => x.Coins).ToImmutableArray() : usedPockets.SelectMany(x => x.Coins).Except(coinsToExclude).ToImmutableArray();
