		...
		public LoadTransactionViewModel(Network network)
		{
			...
			ImportTransactionCommand = ReactiveCommand.CreateFromTask(
				async () =>
				{
					...
					{
						Logger.LogError(ex);
						await ShowErrorAsync(ex.Message, "It was not possible to load the transaction.");
					}
				},
			...
			PasteCommand = ReactiveCommand.CreateFromTask(async () =>
			{
				...
				{
					Logger.LogError(ex);
					await ShowErrorAsync(ex.Message, "It was not possible to paste the transaction.");
				}
			});
		}
