...
public class IndexerTests : IClassFixture<RegTestFixture>
{
	public IndexerTests(RegTestFixture regTestFixture)
	{
		RegTestFixture = regTestFixture;
		IndexerApiHttpClient = regTestFixture.IndexerHttpClientFactory.CreateClient("test");
	}

	private HttpClient IndexerApiHttpClient { get; }
	private RegTestFixture RegTestFixture { get; }

	[Fact]
	public async Task GetClientVersionAsync()
	{
		IndexerClient client = new(IndexerApiHttpClient);
		var indexerCompatible = await client.CheckUpdatesAsync(CancellationToken.None);
		Assert.True(indexerCompatible);
	}
	...
	public async Task BroadcastReplayTxAsync()
	{
		...
		using var response = await IndexerApiHttpClient.PostAsync($"api/v{Constants.IndexerMajorVersion}/btc/blockchain/broadcast", content);
