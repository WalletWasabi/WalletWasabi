...
namespace WalletWasabi.Tests.RegressionTests
{
	...
	public class CoinJoinTests
	{
		...
		public async Task CoordinatorCtorTestsAsync()
		{
			...
			using (var coordinatorToTest = new Coordinator(network, global.HostedServices.First<BlockNotifier>(), folder, rpc, coordinator.RoundConfig))
			{
				...
				var coordinatorToTest2 = new Coordinator(network, global.HostedServices.First<BlockNotifier>(), folder, rpc, coordinatorToTest.RoundConfig);
		...
		public async Task CoinJoinMultipleRoundTestsAsync()
		{
			...
			var httpClientFactory = new HttpClientFactory(torEndPoint: null, backendUriGetter: () => new Uri(RegTestFixture.BackendEndPoint));
			var synchronizer = new WasabiSynchronizer(network, bitcoinStore, httpClientFactory);
			using var feeProvider = new FeeProvider(synchronizer, null);
			...
			using var feeProvider2 = new FeeProvider(synchronizer2, null);
