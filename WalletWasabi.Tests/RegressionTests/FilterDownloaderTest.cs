...
public class FilterDownloaderTest : IClassFixture<RegTestFixture>
{
	...
	public async Task FilterDownloaderTestAsync()
	{
		...
		await using RegTestSetup setup = await RegTestSetup.InitializeTestEnvironmentAsync(RegTestFixture, numberOfBlocksToGenerate: 1, nameof(FilterDownloaderTestAsync));
		...
		using WasabiSynchronizer synchronizer = new(period: TimeSpan.FromSeconds(1), 1000, bitcoinStore, httpClientFactory.SharedWasabiClient);
		try
		{
