...
public class SendSpeedupTests : IClassFixture<RegTestFixture>
{
	...
	public async Task SendSpeedupTestsAsync()
	{
		...
		using CpfpInfoProvider cpfpInfoProvider = new(httpClientFactory);
		...
		WalletFactory walletFactory = new(workDir, network, bitcoinStore, synchronizer, serviceConfiguration, feeProvider, blockDownloadService, cpfpInfoProvider);
		...
		try
		{
			...
			rbf2 = await wallet.SpeedUpTransactionAsync(rbf.Transaction);
			...
			var cpfp = await wallet.SpeedUpTransactionAsync(txToSpeedUp.Transaction);
			...
			rbf = await wallet.SpeedUpTransactionAsync(txToSpeedUp.Transaction, feeRate);
			await broadcaster.SendTransactionAsync(rbf.Transaction);
