...
public class BlockDownloadTests
{
	...
	public async Task BlockDownloadingTestAsync()
	{
		...
		try
		{
			...
			{
				var taskCompletionSource = blockDownloadService.TryGetBlockAsync(P2pSourceRequest.Automatic, blockHash, new Priority(SyncType.Complete, height), testCts.Token);
