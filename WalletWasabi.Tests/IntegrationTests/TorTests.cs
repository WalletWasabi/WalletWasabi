...
namespace WalletWasabi.Tests.IntegrationTests
{
	public class TorTests : IAsyncLifetime
	{
		public TorTests()
		{
			TorHttpPool = new TorHttpPool(new TorTcpConnectionFactory(Common.TorSocks5Endpoint));
		...
		public async Task TorIpIsNotTheRealOneAsync()
		{
			...
			// 2. Get Tor IP
			{
				TorHttpClient torClient = MakeTorHttpClient(new Uri(requestUri));
		...
		public async Task CanRequestOnionV2Async()
		{
			...
			TorHttpClient client = MakeTorHttpClient(new Uri("http://expyuzz4wqqyqhjn.onion/"));
		...
		public async Task IsolatesStreamsAsync()
		{
			...
			TorHttpClient c1 = MakeTorHttpClient(new Uri("http://api.ipify.org"), isolateStream: true);
			TorHttpClient c2 = MakeTorHttpClient(new Uri("http://api.ipify.org"), isolateStream: true);
			TorHttpClient c3 = MakeTorHttpClient(new Uri("http://api.ipify.org"), isolateStream: true);
			Task<HttpResponseMessage> t1 = c1.SendAsync(HttpMethod.Get, "", null, ctsTimeout.Token);
			Task<HttpResponseMessage> t2 = c2.SendAsync(HttpMethod.Get, "", null, ctsTimeout.Token);
			Task<HttpResponseMessage> t3 = c3.SendAsync(HttpMethod.Get, "", null, ctsTimeout.Token);
		...
		{
			return new TorHttpClient(uri, TorHttpPool, isolateStream);
		}
	}
}
