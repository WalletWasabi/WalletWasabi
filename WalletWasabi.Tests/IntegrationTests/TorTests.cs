...
namespace WalletWasabi.Tests.IntegrationTests
{
	public class TorTests : IAsyncLifetime
	{
		...
		{
			var torManager = new TorProcessManager(Global.TorSettings, Global.TorSocks5Endpoint);
		...
		{
			using var client = new TorHttpClient(new Uri("http://anglesharp.azurewebsites.net/"), Global.TorSocks5Endpoint);
		...
		{
			using var client = new TorHttpClient(new Uri("https://postman-echo.com"), Global.TorSocks5Endpoint);
		...
		{
			using var client = new TorHttpClient(new Uri("http://www.dds6qkxpwdeubwucdiaord2xgbbeyds25rbsgr73tbfpqpt4a6vjwsyd.onion"), Global.TorSocks5Endpoint);
			HttpResponseMessage response = await client.SendAsync(HttpMethod.Get, "");
			var content = await response.Content.ReadAsStringAsync();
