...
#pragma warning disable IDE0059 // Value assigned to symbol is never used
		...
		public async Task WalletTestsAsync()
		{
			...
			var key = wallet.GetReceiveKey(new Label("foo label"));
			...
			try
			{
				...
				Assert.Equal(new Label("foo label"), firstCoin.Label);
				...
				Assert.Equal(new Label("foo label"), keyManager.GetKeys(KeyState.Used, false).Single().Label);

				// Get some money, make it confirm.
				var key2 = wallet.GetReceiveKey(new Label("bar label"));
				...
				Assert.Equal(new Label("foo label"), firstCoin.Label);
				Assert.Equal(new Label("bar label"), secondCoin.Label);
				Assert.Equal(new Label("bar label"), thirdCoin.Label);
				Assert.Equal(key.P2wpkhScript, firstCoin.ScriptPubKey);
				...
				Assert.Single(keyManager.GetKeys(x => x.Label == new Label("foo label") && x.KeyState == KeyState.Used && !x.IsInternal));
				Assert.Single(keyManager.GetKeys(x => x.Label == new Label("bar label") && x.KeyState == KeyState.Used && !x.IsInternal));
				...
				Assert.Equal(new Label("bar label"), rbfCoin.Label);
				...
				Assert.Single(keyManager.GetKeys(KeyState.Used, false).Where(x => x.Label == new Label("foo label")));
				Assert.Single(keyManager.GetKeys(KeyState.Used, false).Where(x => x.Label == new Label("bar label")));
		...
		public async Task SendTestsAsync()
		{
			...
			// Get some money, make it confirm.
			var key = wallet.GetReceiveKey(new Label("foo label"));
			var key2 = wallet.GetReceiveKey(new Label("foo label"));
			...
			try
			{
				...
				var res2 = wallet.BuildTransaction(password, new PaymentIntent(scp, Money.Coins(0.05m), label: new Label("foo")), FeeStrategy.CreateFromConfirmationTarget(5), allowUnconfirmed: false);
				...
				Script receive = wallet.GetReceiveKey(new Label("Basic")).P2wpkhScript;
				Money amountToSend = wallet.Coins.Where(x => !x.Unavailable).Sum(x => x.Amount) / 2;
				var res = wallet.BuildTransaction(password, new PaymentIntent(receive, amountToSend, label: new Label("foo")), FeeStrategy.SevenDaysConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				receive = wallet.GetReceiveKey(new Label("SubtractFeeFromAmount")).P2wpkhScript;
				amountToSend = wallet.Coins.Where(x => !x.Unavailable).Sum(x => x.Amount) / 3;
				res = wallet.BuildTransaction(password, new PaymentIntent(receive, amountToSend, subtractFee: true, label: new Label("foo")), FeeStrategy.SevenDaysConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				res = wallet.BuildTransaction(password, new PaymentIntent(receive, amountToSend, label: new Label("foo")), FeeStrategy.SevenDaysConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				res = wallet.BuildTransaction(password, new PaymentIntent(receive, amountToSend, label: new Label("foo")), FeeStrategy.OneDayConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				res = wallet.BuildTransaction(password, new PaymentIntent(receive, amountToSend, label: new Label("foo")), FeeStrategy.TwentyMinutesConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				receive = wallet.GetReceiveKey(new Label("MaxAmount")).P2wpkhScript;

				res = wallet.BuildTransaction(password, new PaymentIntent(receive, MoneyRequest.CreateAllRemaining(), new Label("foo")), FeeStrategy.SevenDaysConfirmationTargetStrategy, allowUnconfirmed: true);
				...
				receive = wallet.GetReceiveKey(new Label("InputSelection")).P2wpkhScript;

				var inputCountBefore = res.SpentCoins.Count();

				res = wallet.BuildTransaction(password, new PaymentIntent(receive, MoneyRequest.CreateAllRemaining(), new Label("foo")), FeeStrategy.SevenDaysConfirmationTargetStrategy,
				...
				Script receive2 = wallet.GetReceiveKey(Label.Empty).P2wpkhScript;
				res = wallet.BuildTransaction(password, new PaymentIntent(receive2, MoneyRequest.CreateAllRemaining(), new Label("my label")), FeeStrategy.SevenDaysConfirmationTargetStrategy, allowUnconfirmed: true);

				Assert.Single(res.InnerWalletOutputs);
				Assert.Equal(new Label("my label"), res.InnerWalletOutputs.Single().Label);

				amountToSend = wallet.Coins.Where(x => !x.Unavailable).Sum(x => x.Amount) / 3;
				res = wallet.BuildTransaction(
					password,
					new PaymentIntent(
						new DestinationRequest(new Key(), amountToSend, label: new Label("outgoing")),
						new DestinationRequest(new Key(), amountToSend, label: new Label("outgoing2"))),
					FeeStrategy.SevenDaysConfirmationTargetStrategy,
					allowUnconfirmed: true);

				Assert.Single(res.InnerWalletOutputs);
				Assert.Equal(2, res.OuterWalletOutputs.Count());
				Assert.Equal(new Label("outgoing, outgoing2"), res.InnerWalletOutputs.Single().Label);

				await wallet.SendTransactionAsync(res.Transaction);

				Assert.Contains(new Label("outgoing, outgoing2"), wallet.Coins.Where(x => x.Height == Height.Mempool).Select(x => x.Label));
				Assert.Contains(new Label("outgoing, outgoing2"), keyManager.GetKeys().Select(x => x.Label));
				...
				Assert.Contains(new Label("outgoing, outgoing2"), wallet.Coins.Where(x => x.Height == bestHeight).Select(x => x.Label));
				Assert.Contains(new Label("outgoing, outgoing2"), keyManager.GetKeys().Select(x => x.Label));
				...
				receive = wallet.GetReceiveKey(new Label("AllowedInputsDisallowUnconfirmed")).P2wpkhScript;

				var allowedInputs = wallet.Coins.Where(x => !x.Unavailable).Select(x => new TxoRef(x.TransactionId, x.Index)).Take(1);
				var toSend = new PaymentIntent(receive, MoneyRequest.CreateAllRemaining(), new Label("fizz"));
				...
				res = wallet.BuildTransaction(
					...
					new PaymentIntent(
						...
						new DestinationRequest(k2, Money.Coins(0.0003m), label: new Label("outgoing"))),
				...
				res = wallet.BuildTransaction(
					password,
					new PaymentIntent(new Key(), Money.Coins(0.0003m), label: new Label("outgoing")),
					FeeStrategy.TwentyMinutesConfirmationTargetStrategy);

				Assert.True(res.FeePercentOfSent > 1);

				var newChangeK = keyManager.GenerateNewKey(new Label("foo"), KeyState.Clean, isInternal: true);
				res = wallet.BuildTransaction(
					password,
					new PaymentIntent(
						new DestinationRequest(newChangeK.P2wpkhScript, MoneyRequest.CreateChange(), new Label("boo")),
						new DestinationRequest(new Key(), Money.Coins(0.0003m), label: new Label("outgoing"))),
					FeeStrategy.TwentyMinutesConfirmationTargetStrategy);
		...
		public async Task BuildTransactionValidationsTestAsync()
		{
			...
			Assert.Throws<ArgumentException>(() => wallet.BuildTransaction(
				password,
				new PaymentIntent(
					new DestinationRequest(scp, MoneyRequest.CreateAllRemaining(), new Label("zero")),
					new DestinationRequest(scp, MoneyRequest.CreateAllRemaining(), new Label("zero"))),
				FeeStrategy.SevenDaysConfirmationTargetStrategy,
				false));
		...
		public async Task SpendUnconfirmedTxTestAsync()
		{
			...
			try
			{
				...
				var to = wallet.GetReceiveKey(new Label("foo"));
				var res = wallet.BuildTransaction(password, new PaymentIntent(to.P2wpkhScript, Money.Coins(0.2345m), label: new Label("bar")), FeeStrategy.TwentyMinutesConfirmationTargetStrategy, allowUnconfirmed: true);
				await wallet.SendTransactionAsync(res.Transaction);
		...
		public async Task CcjFeeTestsAsync()
		{
			...
			for (int i = 0; i < anonymitySet; i++)
			{
				...
				var keyManager = KeyManager.CreateNew(out _, password);
				var key = keyManager.GenerateNewKey(new Label("foo"), KeyState.Clean, false);
		...
		public async Task CcjClientTestsAsync()
		{
			...
			await rpc.GenerateAsync(3); // So to make sure we have enough money.
			var keyManager = KeyManager.CreateNew(out _, password);
			var key1 = keyManager.GenerateNewKey(new Label("foo"), KeyState.Clean, false);
			var key2 = keyManager.GenerateNewKey(new Label("bar"), KeyState.Clean, false);
			var key3 = keyManager.GenerateNewKey(new Label("baz"), KeyState.Clean, false);
			var key4 = keyManager.GenerateNewKey(new Label("qux"), KeyState.Clean, false);
		...
		public async Task CoinJoinMultipleRoundTestsAsync()
		{
			...
			// Get some money, make it confirm.
			var key = wallet.GetReceiveKey(new Label("fundZeroLink"));
			var txId = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), Money.Coins(1m));
			Assert.NotNull(txId);
			var key2 = wallet2.GetReceiveKey(new Label("fundZeroLink"));
			var key3 = wallet2.GetReceiveKey(new Label("fundZeroLink"));
			var key4 = wallet2.GetReceiveKey(new Label("fundZeroLink"));
