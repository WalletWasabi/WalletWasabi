...
public class WabiSabiHttpApiIntegrationTests : IClassFixture<WabiSabiApiApplicationFactory<Startup>>
{
	...
	public async Task SoloCoinJoinTaprootAsync(long[] amounts)
	{
		...
		var coinJoinClient = WabiSabiFactory.CreateTestCoinJoinClient(mockHttpClientFactory.Object, keyManager, roundStateUpdater, random);

		// Run the coinjoin client task.
		var coinjoinResult = await coinJoinClient.StartCoinJoinAsync(coins, cts.Token);
		Assert.IsType<SuccessfulCoinJoinResult>(coinjoinResult);

		var successResult = (SuccessfulCoinJoinResult)coinjoinResult;

		// Scripts should be different.
		Assert.Distinct(successResult.OutputScripts);
