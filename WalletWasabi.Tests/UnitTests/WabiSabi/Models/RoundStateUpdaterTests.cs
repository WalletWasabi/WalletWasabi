...
public class RoundStateUpdaterTests
{
	...
	public async Task CancelAsync()
	{
		...
		using var roundStatusUpdater = Spawn("RoundStateUpdater", EventDriven(
			new RoundsState(new Dictionary<uint256, RoundState>(), ImmutableList<RoundStateAwaiter>.Empty),
			RoundStateUpdater.Create(apiClient)));
...
public static class RoundStateUpdaterExtensions
{
	public static void Update(this MailboxProcessor<RoundUpdateMessage> roundStateUpdater) =>
		roundStateUpdater.Post(new RoundUpdateMessage.UpdateMessage());
}
