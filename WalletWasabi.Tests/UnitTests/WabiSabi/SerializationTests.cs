...
public class SerializationTests
{
	...
	public void IssuanceRequestSerialization()
	{
		...
		var serializedIssuanceRequest = Encode.IssuanceRequest(issuanceRequest).ToJsonString();

		var deserializedIssuanceRequest = Decode.FromString(serializedIssuanceRequest, Decode.IssuanceRequest)!;
	...
	public void CredentialResponseSerialization()
	{
		...
		var serializedCredentialsResponse = Encode.CredentialsResponse(credentialRespose).ToJsonString();

		var deserializedCredentialsResponse = Decode.FromString(serializedCredentialsResponse, Decode.CredentialsResponse)!;
	...
	public void RegistrationMessageSerialization()
	{
		...
		var serializedRequestMessage = Encode.RealCredentialsRequest(realCredentialsRequest).ToJsonString();
		var deserializedCredentialsRequest = Decode.FromString(serializedRequestMessage, Decode.RealCredentialsRequest);

		Assert.NotSame(zeroCredentialsRequest, deserializedCredentialsRequest);

		var deserializedRequestMessage = Decode.FromString(serializedRequestMessage, Decode.RealCredentialsRequest);
		var reserializedRequestMessage = Encode.RealCredentialsRequest(deserializedRequestMessage!).ToJsonString();
		Assert.Equal(serializedRequestMessage, reserializedRequestMessage);

		// Registration response message.
		var serializedResponseMessage = Encode.CredentialsResponse(credentialResponse).ToJsonString();
		var deserializedResponseMessage = Decode.FromString(serializedResponseMessage, Decode.CredentialsResponse);
		var reserializedResponseMessage = Encode.CredentialsResponse(deserializedResponseMessage).ToJsonString();
