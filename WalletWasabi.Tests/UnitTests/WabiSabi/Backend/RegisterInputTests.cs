...
namespace WalletWasabi.Tests.UnitTests.WabiSabi.Backend
{
	public class RegisterInputTests
	{
		...
		private static MockRpcClient GetMockRpc(Key? key = null)
		{
			...
			rpc.OnGetTxOutAsync = (_, _, _) => new()
			{
				...
				TxOut = new TxOut(Money.Coins(1), key?.PubKey.GetSegwitAddress(Network.Main).ScriptPubKey ?? Script.Empty)
			};
		...
		public async Task SuccessAsync()
		{
			...
			await using PostRequestHandler handler = new(cfg, new Prison(), arena, GetMockRpc(key));
		...
		public async Task RoundNotFoundAsync()
		{
			...
			await using PostRequestHandler handler = new(new WabiSabiConfig(), new Prison(), arena, GetMockRpc(key));
		...
		public async Task NonUniqueInputsAsync()
		{
			...
			await using PostRequestHandler handler = new(cfg, new Prison(), arena, GetMockRpc());
		...
		public async Task InputBannedAsync()
		{
			...
			var round = WabiSabiFactory.CreateRound(cfg);
			using Arena arena = await WabiSabiFactory.CreateAndStartArenaAsync(cfg, round);
			await using PostRequestHandler handler = new(cfg, prison, arena, GetMockRpc());
