...
public class UtxoPrisonWardenTests
{
	...
	public async Task PrisonSerializationAsync()
	{
		...
		w.Prison.FailedToConfirm(i2, Money.Coins(1m), uint256.One);
		w.Prison.FailedToSign(i3, Money.Coins(1m), uint256.One);
		w.Prison.DoubleSpent(i4, Money.Coins(1m), uint256.One);
