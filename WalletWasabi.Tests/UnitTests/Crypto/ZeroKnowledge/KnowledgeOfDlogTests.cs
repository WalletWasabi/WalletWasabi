...
namespace WalletWasabi.Tests.UnitTests.Crypto.ZeroKnowledge
{
	public class KnowledgeOfDLogTests
	{
		...
		public void End2EndVerificationSimple(uint scalarSeed)
		{
			...
			var statement = new LegacyStatement(publicPoint, generator);
		...
		public void End2EndVerificationLargeScalar()
		{
			...
			var p = secret * gen;
			var statement = new LegacyStatement(p, gen);
			var knowledgeParams = new KnowledgeOfDlogParams(secret, statement);
			var proof = Prover.CreateProof(knowledgeParams);
			Assert.True(Verifier.Verify(proof, statement));
			...
			p = secret * gen;
			statement = new LegacyStatement(p, gen);
		...
		public void BuildChallenge()
		{
			...
			nonce = point2;
			statement = new LegacyStatement(publicPoint, generator);
		...
		public void KnowledgeOfDlogParamsThrows()
		{
			...
			// Demonstrate when it shouldn't throw.
			new KnowledgeOfDlogParams(two, new LegacyStatement(two * Generators.G, Generators.G));

			// Zero cannot pass through.
			Assert.ThrowsAny<ArgumentException>(() => new KnowledgeOfDlogParams(Scalar.Zero, new LegacyStatement(Generators.G, Generators.G)));

			// Public point must be generator * secret.
			Assert.ThrowsAny<InvalidOperationException>(() => new KnowledgeOfDlogParams(two, new LegacyStatement(Generators.G, Generators.G)));
			Assert.ThrowsAny<InvalidOperationException>(() => new KnowledgeOfDlogParams(two, new LegacyStatement(new Scalar(3) * Generators.G, Generators.G)));
			Assert.ThrowsAny<InvalidOperationException>(() => new KnowledgeOfDlogParams(two, new LegacyStatement(Scalar.One * Generators.G, Generators.G)));

			// Secret cannot overflow.
			Assert.ThrowsAny<ArgumentException>(() => new KnowledgeOfDlogParams(EC.N, new LegacyStatement(EC.N * Generators.G, Generators.G)));
			Assert.ThrowsAny<ArgumentException>(() => new KnowledgeOfDlogParams(CryptoHelpers.ScalarLargestOverflow, new LegacyStatement(CryptoHelpers.ScalarLargestOverflow * Generators.G, Generators.G)));
		}
	}
}
