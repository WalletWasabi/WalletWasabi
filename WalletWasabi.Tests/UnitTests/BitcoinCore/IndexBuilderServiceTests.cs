...
{
	[Fact]
	public async Task V0KeyhashUnsynchronizedBitcoinNodeAsync()
	{
	...
	[Fact]
	public async Task V0KeyhashStalledBitcoinNodeAsync()
	{
		var called = 0;
		var rpc = new MockRpcClient
		{
	...
	[Fact]
	public async Task V0KeyhashSynchronizingBitcoinNodeAsync()
	{
		var called = 0;
	...
	[Fact]
	public async Task V0KeyhashSynchronizedBitcoinNodeAsync()
	{
		var blockchain = GenerateBlockchain().Take(10).ToArray();
		var rpc = new MockRpcClient
		{
	...
	[Fact]
	public async Task V1TaprootUnsynchronizedBitcoinNodeAsync()
	{
		var rpc = new MockRpcClient
		{
			OnGetBlockchainInfoAsync = () => Task.FromResult(new BlockchainInfo
		...
		using var blockNotifier = new BlockNotifier(TimeSpan.MaxValue, rpc);
		var indexer = new IndexBuilderService(RpcPubkeyType.TxWitnessV1Taproot, rpc, blockNotifier, "filters.txt");
	...
	[Fact]
	public async Task V1TaprootStalledBitcoinNodeAsync()
	{
		var called = 0;
		var rpc = new MockRpcClient
		{
	...
	[Fact]
	public async Task V1TaprootSynchronizingBitcoinNodeAsync()
	{
		var called = 0;
		var blockchain = GenerateBlockchain().Take(10).ToArray();
		var rpc = new MockRpcClient
	...
	[Fact]
	public async Task V1TaprootSynchronizedBitcoinNodeAsync()
	{
		var blockchain = GenerateBlockchain().Take(10).ToArray();
		var rpc = new MockRpcClient
		{
