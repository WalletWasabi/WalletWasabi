	...
	public class RpcBasedTests
	{
		...
		public async Task CantDoubleSpendAsync()
		{
			...
			try
			{
				...
				_ = tx.Inputs.Add(coinBaseTx, 0);
				_ = tx.Outputs.Add(Money.Coins(49.9999m), new Key().PubKey.WitHash.GetAddress(network));
				tx.Sign(key.GetBitcoinSecret(network), coinBaseTx.Outputs.AsCoins().First());
				var valid = tx.Check();

				var doubleSpend = Transaction.Create(network);
				_ = doubleSpend.Inputs.Add(coinBaseTx, 0);
				_ = doubleSpend.Outputs.Add(Money.Coins(49.998m), new Key().PubKey.WitHash.GetAddress(network));
				...
				_ = await Assert.ThrowsAsync<RPCException>(async () => await rpc.SendRawTransactionAsync(doubleSpend));
			}
			finally
			{
				await services.StopAllAsync(CancellationToken.None);
		...
		public void ParseVerboseBlockInfo()
		{
			...
			_ = Assert.Single(blockInfo.Transactions.ElementAt(0).Inputs);
			Assert.Equal(2, blockInfo.Transactions.ElementAt(0).Outputs.Count());
			_ = Assert.Single(blockInfo.Transactions.ElementAt(1).Inputs);
