...
public class BlockDownloadServiceTests
{
	...
	public async Task TryGetBlockTests1Async()
	{
		...
		using (BlockDownloadService service = new(fileSystemBlockRepository, [fullNodeBlockProvider], p2pBlockProvider: null, maximumParallelTasks: 3))
		{
			...
			var task1 = service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash1, new Priority(SyncType.Complete, BlockHeight: 610_001), testCts.Token);
			var task2 = service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash2, new Priority(SyncType.Complete, BlockHeight: 610_002), testCts.Token);
			var task3 = service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash3, new Priority(SyncType.Complete, BlockHeight: 610_003), testCts.Token);
			var task4 = service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash4, new Priority(SyncType.Complete, BlockHeight: 610_004), testCts.Token);
			...
			{
				var task2Result = await service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash2, new Priority(SyncType.Complete, BlockHeight: 610_002), testCts.Token);
	...
	public async Task TryGetBlockTests2Async()
	{
		...
		using (BlockDownloadService service = new(fileSystemBlockRepository, [fullNodeBlockProvider], p2pBlockProvider: null, maximumParallelTasks: 3))
		{
			...
			var actualResult1 = await service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash1, new Priority(SyncType.Complete, BlockHeight: 610_001), testCts.Token);
			Assert.True(actualResult1.IsOk);
			Assert.Same(block1, actualResult1.Value);

			var actualResult2 = await service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash2, new Priority(SyncType.Complete, BlockHeight: 610_002), testCts.Token);
			Assert.False(actualResult2.IsOk);

			var actualResult3 = await service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash3, new Priority(SyncType.Complete, BlockHeight: 610_003), testCts.Token);
			Assert.True(actualResult3.IsOk);
			Assert.Same(block3, actualResult3.Value);

			var actualResult4 = await service.TryGetBlockAsync(TrustedFullNodeSourceRequest.Instance, blockHash4, new Priority(SyncType.Complete, BlockHeight: 610_004), testCts.Token);
			...
			var actualResult5 = await service.TryGetBlockAsync(P2pSourceRequest.Automatic, blockHash2, new Priority(SyncType.Complete, BlockHeight: 610_005), testCts.Token);
