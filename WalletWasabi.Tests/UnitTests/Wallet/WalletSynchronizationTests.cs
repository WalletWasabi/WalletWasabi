...
public class WalletSynchronizationTests
{
	...
	public async Task InternalAddressReuseNoBlockOverlapTestAsync()
	{
		...
		await realWallet.PerformWalletSynchronizationAsync(SyncType.Turbo, null, testDeadlineCts.Token);
		Assert.Single(coins.AsAllCoinsView());

		await realWallet.PerformWalletSynchronizationAsync(SyncType.NonTurbo, null, testDeadlineCts.Token);
