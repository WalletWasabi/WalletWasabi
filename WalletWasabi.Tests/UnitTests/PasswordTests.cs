...
namespace WalletWasabi.Tests.UnitTests
{
	public class PasswordTests
	{
		...
		public void FormattingTest()
		{
			...
			Assert.Throws<SecurityException>(() => PasswordHelper.GetMasterExtKey(keyManager, PseudoRandom.GetString(PasswordHelper.MaxPasswordLength), out _));

			// Password should be formatted, before entering here.
			Assert.Throws<FormatException>(() => PasswordHelper.GetMasterExtKey(keyManager, PseudoRandom.GetString(PasswordHelper.MaxPasswordLength + 1), out _));

			// Too long password with extra spaces.
			var badPassword = $"   {PseudoRandom.GetString(PasswordHelper.MaxPasswordLength + 1)}   ";
