...
namespace WalletWasabi.Tests.UnitTests
{
	public class PasswordTests
	{
		...
		public void FormattingTest()
		{
			...
			Assert.Throws<SecurityException>(() => PasswordHelper.GetMasterExtKey(keyManager, (new UnsecureRandom() as IWasabiRandom).GetString(PasswordHelper.MaxPasswordLength, Characters.AlphaNumeric), out _));

			// Password should be formatted, before entering here.
			Assert.Throws<FormatException>(() => PasswordHelper.GetMasterExtKey(keyManager, (new UnsecureRandom() as IWasabiRandom).GetString(PasswordHelper.MaxPasswordLength + 1, Characters.AlphaNumeric), out _));

			// Too long password with extra spaces.
			var badPassword = $"   {(new UnsecureRandom() as IWasabiRandom).GetString(PasswordHelper.MaxPasswordLength + 1, Characters.AlphaNumeric)}   ";
