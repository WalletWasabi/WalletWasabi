...
public class AllTransactionStoreTests
{
	...
	private void PrepareTestEnv(out Network network, out SmartTransaction uTx1, out SmartTransaction uTx2, out SmartTransaction uTx3, out SmartTransaction cTx1, out SmartTransaction cTx2, out SmartTransaction cTx3)
	{
	...
	public async Task CanInitializeEmptyAsync(Network network)
	{
		...
		Assert.Empty(txStore.GetTransactions(TransactionType.Mempool));
		Assert.Empty(txStore.GetTransactionHashes(TransactionType.Mempool));
		Assert.Empty(txStore.GetTransactions(TransactionType.Confirmed));
		Assert.Empty(txStore.GetTransactionHashes(TransactionType.Confirmed));
	...
	public async Task CanInitializeAsync()
	{
		...
		Assert.Equal(3, txStore.GetTransactions(TransactionType.Mempool).Count());
		Assert.Equal(3, txStore.GetTransactionHashes(TransactionType.Mempool).Count());
		Assert.Equal(3, txStore.GetTransactions(TransactionType.Confirmed).Count());
		Assert.Equal(3, txStore.GetTransactionHashes(TransactionType.Confirmed).Count());
	...
	{
		PrepareTestEnv(out Network network, out SmartTransaction uTx1, out SmartTransaction uTx2, out SmartTransaction uTx3, out SmartTransaction cTx1, out SmartTransaction cTx2, out SmartTransaction cTx3);
	...
	public async Task ReorgAsync()
	{
		...
		Assert.True(txStore.TryGetMempoolTransaction(uTx1.GetHash(), out var myUnconfirmedTx1));
		Assert.False(myUnconfirmedTx1.Confirmed);
		Assert.True(txStore.TryGetMempoolTransaction(uTx2.GetHash(), out var myUnconfirmedTx2));
		...
		Assert.True(txStore.TryGetConfirmedTransaction(uTx1.GetHash(), out var myTx1));
		Assert.False(txStore.TryGetMempoolTransaction(uTx1.GetHash(), out _));
		Assert.True(myTx1.Confirmed);
		Assert.True(txStore.TryGetConfirmedTransaction(uTx2.GetHash(), out var myTx2));
		Assert.False(txStore.TryGetMempoolTransaction(uTx2.GetHash(), out _));
		...
		// Two transactions are in the mempool store and unconfirmed.
		Assert.True(txStore.TryGetMempoolTransaction(uTx1.GetHash(), out var myReorgedTx1));
		Assert.False(txStore.TryGetConfirmedTransaction(uTx1.GetHash(), out _));
		Assert.False(myReorgedTx1?.Confirmed);
		Assert.True(txStore.TryGetMempoolTransaction(uTx2.GetHash(), out var myReorgedTx2));
		Assert.False(txStore.TryGetConfirmedTransaction(uTx2.GetHash(), out _));
