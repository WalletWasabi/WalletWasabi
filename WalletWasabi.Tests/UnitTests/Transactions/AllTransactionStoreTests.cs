...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class AllTransactionStoreTests
	{
		...
		{
			string dir = Path.Combine(Common.DataDir, EnvironmentHelpers.ExtractFileName(callerFilePath), callerMemberName);
		...
		public async Task CanInitializeEmptyAsync(Network network)
		{
			...
			uint256 txHash = Common.GenerateRandomSmartTransaction().GetHash();
		...
		public async Task CanInitializeAsync()
		{
			...
			uint256 doesntContainTxHash = Common.GenerateRandomSmartTransaction().GetHash();
