...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class AllTransactionStoreTests
	{
		...
		{
			var txStore = new AllTransactionStore(network);
			var dir = PrepareWorkDir();
			await txStore.InitializeAsync(dir, ensureBackwardsCompatibility: false);
		...
		public async Task CorrectsOrderAsync()
		{
			...
			txStore = new AllTransactionStore(network);
			await txStore.InitializeAsync(PrepareWorkDir(), ensureBackwardsCompatibility: false);
