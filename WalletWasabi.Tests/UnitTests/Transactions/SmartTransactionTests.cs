...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class SmartTransactionTests
	{
		...
		public void SmartTransactionJsonSerialization()
		{
			...
			var serialized = JsonSerializer.Serialize(smartTx);
			var deserialized = JsonSerializer.Deserialize<SmartTransaction>(serialized);
			Assert.Equal(label, smartTx.Label);

			var serialized2 = JsonSerializer.Serialize(smartTx2);
			var deserialized2 = JsonSerializer.Deserialize<SmartTransaction>(serialized2);

			var serialized3 = JsonSerializer.Serialize(smartTx3);
			var deserialized3 = JsonSerializer.Deserialize<SmartTransaction>(serialized3);
			...
			var deserializedWithoutLabel = JsonSerializer.Deserialize<SmartTransaction>(serializedWithoutLabel);
		...
		public void FirstSeenBackwardsCompatibility()
		{
			...
			deserialized = JsonSerializer.Deserialize<SmartTransaction>(serialized);
			Assert.Equal(now.UtcDateTime, deserialized.FirstSeen.UtcDateTime, TimeSpan.FromSeconds(1));
