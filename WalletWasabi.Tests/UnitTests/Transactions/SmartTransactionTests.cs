...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class SmartTransactionTests
	{
		...
		public void SmartTransactionLineDeserialization()
		{
			...
			_ = Assert.Throws<ArgumentNullException>(() => SmartTransaction.FromLine(input, network));
			network = Network.Main;
			input = "";
			_ = Assert.Throws<ArgumentException>(() => SmartTransaction.FromLine(input, network));
			input = " ";
			_ = Assert.Throws<ArgumentException>(() => SmartTransaction.FromLine(input, network));
			input = null;
			_ = Assert.Throws<ArgumentNullException>(() => SmartTransaction.FromLine(input, network));
