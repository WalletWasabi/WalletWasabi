...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class TransactionFactoryTests
	{
		...
		public void SpendOnlyAllowedCoins()
		{
			...
			var result = transactionFactory.BuildTransaction(payment, feeRate, allowedCoins.Select(x => x.GetOutPoint()));
		...
		public void InsufficientAllowedCoins()
		{
			...
			var ex = Assert.Throws<InsufficientBalanceException>(() =>
				transactionFactory.BuildTransaction(payment, new FeeRate(2m), allowedCoins.Select(x => x.GetOutPoint())));
		...
		public void SpendWholeCoinsEvenWhenNotAllowed()
		{
			...
			{
				coins.Single(x => x.Amount == Money.Coins(0.08m)).GetOutPoint(),
				coins.Single(x => x.Amount == Money.Coins(0.02m)).GetOutPoint()
			}.ToArray();
