...
namespace WalletWasabi.Tests.UnitTests.Transactions
{
	public class AnonymityEstimationTests
	{
		...
		{
			var coin = Common.RandomSmartCoin(keyManager.GenerateNewKey("", KeyState.Used, false), value.ToDecimal(MoneyUnit.BTC), false, anonset);
			registry.TryAdd(coin);
			return coin;
		}

		private static void GetNewServices(out KeyManager keyManager, out CoinsRegistry registry, out AnonymityEstimator estimator)
		{
			keyManager = KeyManager.CreateNew(out _, "");
			registry = new CoinsRegistry(1);
			estimator = new AnonymityEstimator(registry, keyManager, Money.Satoshis(1));
		}
		...
		private static Transaction GetNewInputTransaction(IEnumerable<SmartCoin> inputCoins, int randomInputs)
		{
			...
			{
				tx.Inputs.Add(new OutPoint(Common.RandomUint256(), i));
			}
