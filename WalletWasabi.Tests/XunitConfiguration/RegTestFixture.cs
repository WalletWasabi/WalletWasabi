	...
	public class RegTestFixture : IDisposable
	{
			...
			int maximumMixingLevelCount)
		{
			return new CoordinatorRoundConfig
			{
				...
				InputRegistrationTimeout = TimeSpan.FromSeconds(inputRegistrationTimeout),
				ConnectionConfirmationTimeout = TimeSpan.FromSeconds(connectionConfirmationTimeout),
				SigningTimeout = TimeSpan.FromSeconds(signingTimeout),
				OutputRegistrationTimeout = TimeSpan.FromSeconds(outputRegistrationTimeout),
