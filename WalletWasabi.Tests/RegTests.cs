	...
	public class RegTests : IClassFixture<SharedFixture>
	{
		...
		public async Task MempoolAsync()
		{
			...
			try
			{
				...
				{
					var addr = await rpc.GetNewAddressAsync();
					var res = await rpc.SendToAddressAsync(addr, new Money(0.01m, MoneyUnit.BTC));
					Assert.NotNull(res);
				}
		...
		public async Task ReorgTestAsync()
		{
			...
			var tx2 = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			key = keyManager.GenerateNewKey("", KeyState.Clean, isInternal: false);
			var tx3 = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			var tx4 = await rpc.SendToAddressAsync(key.GetP2pkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			var tx5 = await rpc.SendToAddressAsync(key.GetP2shOverP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			var tx1 = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC), replaceable: true);
		...
		public async Task WalletTestsAsync()
		{
			...
			var txid = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			await rpc.GenerateAsync(1);
			...
			try
			{
				...
				// TEST MEMPOOL
				var txid5 = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
		...
		public async Task BuildTransactionReorgsTestAsync()
		{
			...
			// Get some money, make it confirm.
			var key = wallet.GetReceiveKey("foo label");
			var fundingTxid = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(0.1m, MoneyUnit.BTC));
			...
			try
			{
				...
				fundingTxid = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(1m, MoneyUnit.BTC), replaceable: true);
		...
		public async Task SpendUnconfirmedTxTestAsync()
		{
			...
			try
			{
				...
				// Get some money, make it confirm.
				// this is necesary because we are in a fork now.
				var tx0Id = await rpc.SendToAddressAsync(key.GetP2wpkhAddress(network), new Money(1m, MoneyUnit.BTC),
					replaceable: true);
		...
		public async Task CcjTestsAsync()
		{
			...
			using (var satoshiClient = new SatoshiClient(baseUri))
			{
				...
				var index = tx.Outputs.GetIndexes(addr.ScriptPubKey).Single();
				...
				index = tx.Outputs.GetIndexes(witnessAddress.ScriptPubKey).Single();
				inputsRequest.Inputs = new List<InputProofModel> { new InputProofModel { Input = new OutPoint(hash, index), Proof = "b" } };
				httpRequestException = await Assert.ThrowsAsync<HttpRequestException>(async () => await AliceClient.CreateNewAsync(inputsRequest, baseUri));
				...
				using (var aliceClient = await AliceClient.CreateNewAsync(new Key().PubKey.GetAddress(network), BlindedData, new InputProofModel[] { new InputProofModel { Input = new OutPoint(hash, index), Proof = key.SignMessage(ByteHelpers.ToHex(BlindedData)) } }, baseUri))
				{
					Assert.NotNull(aliceClient.BlindedOutputSignature);
					Assert.NotEqual(Guid.Empty, aliceClient.UniqueId);
					Assert.True(aliceClient.RoundId > 0);
		...
		public async Task Ccj100ParticipantsTestsAsync()
		{
			...
			for (int i = 0; i < roundConfig.AnonymitySet; i++)
			{
				...
				for (int j = 0; j < numberOfInputs; j++)
				{
					...
					var outputIndex = transaction.Outputs.GetIndexes(inputAddress.ScriptPubKey).Single();
		...
		public async Task CcjClientTestsAsync()
		{
			...
			var smartCoin1 = new SmartCoin(txid1, tx1.Outputs.GetIndexes(bech1.ScriptPubKey).Single(), bech1.ScriptPubKey, amount1, tx1.Inputs.Select(x => new TxoRef(x.PrevOut)).ToArray(), height, rbf: false, mixin: tx1.GetMixin(tx1.Outputs.GetIndexes(bech1.ScriptPubKey).Single()));
			var smartCoin2 = new SmartCoin(txid2, tx2.Outputs.GetIndexes(bech2.ScriptPubKey).Single(), bech2.ScriptPubKey, amount2, tx2.Inputs.Select(x => new TxoRef(x.PrevOut)).ToArray(), height, rbf: false, mixin: tx2.GetMixin(tx2.Outputs.GetIndexes(bech2.ScriptPubKey).Single()));
			var smartCoin3 = new SmartCoin(txid3, tx3.Outputs.GetIndexes(bech3.ScriptPubKey).Single(), bech3.ScriptPubKey, amount3, tx3.Inputs.Select(x => new TxoRef(x.PrevOut)).ToArray(), height, rbf: false, mixin: tx3.GetMixin(tx3.Outputs.GetIndexes(bech3.ScriptPubKey).Single()));
		...
		public async Task CoinJoinMultipleRoundTestsAsync()
		{
			...
			var txid2 = await rpc.SendToAddressAsync(key2.GetP2wpkhAddress(network), new Money(0.11m, MoneyUnit.BTC));
			var txid3 = await rpc.SendToAddressAsync(key3.GetP2wpkhAddress(network), new Money(0.12m, MoneyUnit.BTC));
			var txid4 = await rpc.SendToAddressAsync(key4.GetP2wpkhAddress(network), new Money(0.13m, MoneyUnit.BTC));
