	...
	public class RegTests : IClassFixture<SharedFixture>
	{
		...
		public async Task WalletTestsAsync()
		{
			...
			try
			{
				...
				Assert.Equal(indexDownloader.BestFilter.BlockHeight, firstCoin.Height);
				...
				Assert.Equal(indexDownloader.BestFilter.BlockHeight.Value - 2, firstCoin.Height.Value);
				Assert.Equal(indexDownloader.BestFilter.BlockHeight.Value - 1, secondCoin.Height.Value);
				Assert.Equal(indexDownloader.BestFilter.BlockHeight, thirdCoin.Height);
				...
				Assert.Equal(indexDownloader.BestFilter.BlockHeight.Value - 2, rbfCoin.Height.Value);
				...
				Assert.Equal(indexDownloader.BestFilter.BlockHeight, mempoolCoin.Height);
			}
			finally
			{
				wallet.NewFilterProcessed -= Wallet_NewFilterProcessed;
		...
		public async Task SendTestsFromHiddenWalletAsync() // These tests are taken from HiddenWallet, they were tests on the testnet.
		{
			...
			try
			{
				...
				Money amountToSend = wallet.Coins.Where(x => !x.SpentOrCoinJoinInProcess).Sum(x => x.Amount) / 2;
				...
				Assert.Equal(wallet.Coins.Where(x => !x.SpentOrCoinJoinInProcess).Sum(x => x.Amount) - res.Fee, maxBuiltTxOutput.Value);
				...
				res = await wallet.BuildTransactionAsync(password, new[] { new WalletService.Operation(receive, Money.Zero, "foo") }, 1008,
					...
					allowedInputs: wallet.Coins.Where(x => !x.SpentOrCoinJoinInProcess).Select(x => new TxoRef(x.TransactionId, x.Index)).Take(1));
				...
				var bestHeight = wallet.IndexDownloader.BestFilter.BlockHeight;
				...
				var allowedInputs = wallet.Coins.Where(x => !x.SpentOrCoinJoinInProcess).Select(x => new TxoRef(x.TransactionId, x.Index)).Take(1);
		...
		public async Task SpendUnconfirmedTxTestAsync()
		{
			...
			try
			{
				...
				Assert.Equal(1, wallet.Coins.Count(x => x.SpentOrCoinJoinInProcess && x.SpenderTransactionId == tx1Res.Transaction.GetHash()));
				...
				var totalWallet = wallet.Coins.Where(c => !c.SpentOrCoinJoinInProcess).Sum(c => c.Amount);
				...
				// There is a coin destroyed
				Assert.Equal(1, wallet.Coins.Count(x => x.SpentOrCoinJoinInProcess && x.SpenderTransactionId == tx2Res.Transaction.GetHash()));

				// There is at least one coin created from the destruction of the first coin
				Assert.Contains(wallet.Coins, x => x.SpentOutputs.Any(o => o.TransactionId == tx1Res.Transaction.GetHash()));

				totalWallet = wallet.Coins.Where(c => !c.SpentOrCoinJoinInProcess).Sum(c => c.Amount);
		...
		public async Task CcjTestsAsync()
		{
			...
			using (var satoshiClient = new SatoshiClient(baseUri))
			{
				...
				using (var aliceClient = await AliceClient.CreateNewAsync(new Key().PubKey.GetAddress(network), BlindedData, new InputProofModel[] { new InputProofModel { Input = coin.Outpoint.ToTxoRef(), Proof = key.SignMessage(ByteHelpers.ToHex(BlindedData)) } }, baseUri))
				{
					Assert.NotNull(aliceClient.BlindedOutputSignature);
					Assert.NotEqual(Guid.Empty, aliceClient.UniqueId);
					Assert.True(aliceClient.RoundId > 0);
				...
				using (var aliceClient1 = await AliceClient.CreateNewAsync(new Key().PubKey.GetAddress(network), blinded1.BlindedData, new InputProofModel[] { new InputProofModel { Input = input1.ToTxoRef(), Proof = key1.SignMessage(blindedOutputScriptHex1) } }, baseUri))
				using (var aliceClient2 = await AliceClient.CreateNewAsync(new Key().PubKey.GetAddress(network), blinded2.BlindedData, new InputProofModel[] { new InputProofModel { Input = input2.ToTxoRef(), Proof = key2.SignMessage(blindedOutputScriptHex2) } }, baseUri))
				{
		...
		public async Task CcjClientTestsAsync()
		{
			...
			try
			{
				...
				Assert.True(smartCoin1.CoinJoinInProcess);
				...
				Assert.True(smartCoin1.CoinJoinInProcess);
				Assert.False(smartCoin2.CoinJoinInProcess);
				smartCoin1.CoinJoinInProcess = false;

				await chaumianClient1.QueueCoinsToMixAsync(password, smartCoin1, smartCoin2);
				Assert.True(smartCoin1.CoinJoinInProcess);
				Assert.True(smartCoin2.CoinJoinInProcess);
				...
				Assert.False(smartCoin1.CoinJoinInProcess);
		...
		public async Task CoinJoinMultipleRoundTestsAsync()
		{
			...
			try
			{
				...
				Assert.Equal(3, wallet.Coins.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.SpentOrCoinJoinInProcess));
				Assert.Equal(3, wallet2.Coins.Count(x => x.Label == "ZeroLink Mixed Coin" && !x.SpentOrCoinJoinInProcess));
				...
				Assert.Equal(1, wallet.Coins.Count(x => x.Label == "ZeroLink Dequeued Change" && !x.SpentOrCoinJoinInProcess));
			}
			finally
			{
				wallet.NewFilterProcessed -= Wallet_NewFilterProcessed;
