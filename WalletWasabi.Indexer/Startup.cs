...
using WalletWasabi.Indexer.Middlewares;
...
public class Startup
{
	...
	{
		string dataDir = Configuration["datadir"] ?? EnvironmentHelpers.GetDataDir(Path.Combine("WalletWasabi", "Indexer"));
		Logger.InitializeDefaults(Path.Combine(dataDir, "Logs.txt"));

		services.AddMemoryCache();
		services.AddMvc(options =>
			{
				options.OutputFormatters.Insert(0, new WasabiJsonOutputFormatter(Encode.IndexerMessage));
