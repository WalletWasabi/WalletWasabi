	...
	internal class LoadWalletViewModel : CategoryViewModel
	{
		...
		public async Task LoadWalletAsync()
		{
			try
			{
				...
				try
				{
					await Task.Run(async () => await Global.CreateWalletServiceAsync(keyManager));
					// Successfully initialized.
					Owner.OnClose();
					// Open Wallet Explorer tabs
					if (Global.DefaultWalletService.Coins.Any())
					{
						// If already have coins then open the last active tab first.
						IoC.Get<WalletExplorerViewModel>().OpenWallet(Global.DefaultWalletService, receiveDominant: false);
					}
					else // Else open with Receive tab first.
					{
						IoC.Get<WalletExplorerViewModel>().OpenWallet(Global.DefaultWalletService, receiveDominant: true);
					}
				}
				catch (Exception ex)
				{
