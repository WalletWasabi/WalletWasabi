...
namespace WalletWasabi.Gui.Controls.WalletExplorer
{
	public abstract class SendControlViewModel : WalletActionViewModel
	{
			...
			: base(title, walletViewModel)
		{
			...
			CoinList = new CoinListViewModel(Wallet.WalletService, displayCommonOwnershipWarning: true);
			...
			BuildTransactionCommand = ReactiveCommand.CreateFromTask(async () =>
			{
				try
				{
					...
					try
					{
						...
						{
							await Wallet.WalletService.ChaumianClient.DequeueCoinsFromMixAsync(toDequeue, DequeueReason.TransactionBuilding);
						}
					}
					catch
					{
					...
					BuildTransactionResult result = await Task.Run(() => Wallet.WalletService.BuildTransaction(Password, intent, feeStrategy, allowUnconfirmed: true, allowedInputs: selectedCoinReferences));
