...
namespace WalletWasabi.Gui.Controls.WalletExplorer
{
    public class SendTabViewModel : WalletActionViewModel
    {
        ...
        private FeeRate _satoshiPerByteFeeRate;
            ...
            : base(isTransactionBuilder ? "Build Transaction" : "Send", walletViewModel)
        {
            ...
            BuildTransactionCommand = ReactiveCommand.CreateFromTask(async () =>
            {
                try
                {
                    ...
                    if (SatoshiPerByteFeeRate is null || SatoshiPerByteFeeRate.SatoshiPerByte < 1)
                    {
                    ...
                    FeeRate feeRate = SatoshiPerByteFeeRate;
        ...
        private void SetFeesAndTexts()
        {
            ...
            else
            {
                ...
                if (allFeeEstimate != null)
                {
                    ...
                    {
                        SatoshiPerByteFeeRate = new FeeRate(userFee);
                        ...
                        foreach (var feeEstimate in allFeeEstimate.Estimations)
                        {
                            ...
                            if (SatoshiPerByteFeeRate.SatoshiPerByte > fee)
                            {
                                feeTarget = target;
                                break;
                            }
                ...
                else
                {
                    switch (FeeDisplayFormat)
                    {
                        case FeeDisplayFormat.SatoshiPerByte:
                            FeeText = $"(~ {SatoshiPerByteFeeRate.SatoshiPerByte} sat/byte)";
        ...
        private void SetFees(AllFeeEstimate allFeeEstimate, int feeTarget)
        {
            if (IsSliderFeeUsed)
            {
                ...
                UserFeeText = SatoshiPerByteFeeRate.SatoshiPerByte.ToString();
            }
            ...
            {
                EstimatedBtcFee = SatoshiPerByteFeeRate.GetTotalFee(vsize);
            }
            else
            {
        ...
        public FeeRate SatoshiPerByteFeeRate
        {
