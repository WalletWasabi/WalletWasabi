	...
	public class CoinViewModel : ViewModelBase, IDisposable
	{
		...
		public CoinViewModel(CoinListViewModel owner, SmartCoin model)
		{
			...
			Observable
				...
				.Merge(Observable.FromEventPattern(Owner.Wallet.WalletService.ChaumianClient, nameof(owner.Wallet.WalletService.ChaumianClient.StateUpdated)).Select(_ => Unit.Default))
				.ObserveOn(RxApp.MainThreadScheduler)
		...
		private SmartCoinStatus GetSmartCoinStatus()
		{
			...
			if (Model.CoinJoinInProgress && Owner.Wallet.WalletService.ChaumianClient != null)
			{
				ClientState clientState = Owner.Wallet.WalletService.ChaumianClient.State;
