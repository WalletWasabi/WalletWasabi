...
namespace WalletWasabi.Gui
{
	public class MainWindow : MetroWindow
	{
		...
		private async Task ClosingAsync()
		{
			...
			try
			{
				if (Global.DefaultChaumianClient != null)
				{
					Global.DefaultChaumianClient.IsQuitPending = true; // indicate -> do not add any more alices to the coinjoin
				}
			...
			finally
			{
				if (!closeApplication) //we are not closing the application for some reason
				{
					...
					if (Global.DefaultChaumianClient != null)
					{
						Global.DefaultChaumianClient.IsQuitPending = false; //re-enable enqueuing coins
					}
				}
			}
		}
