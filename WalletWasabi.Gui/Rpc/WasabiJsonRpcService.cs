...
namespace WalletWasabi.Gui.Rpc
{
	public partial class WasabiJsonRpcService
	{
		...
		public object[] GetUnspentCoinList()
		{
			...
			return Global.GetDefaultWallet().WalletService.Coins.Where(x => x.Unspent).Select(x => new
			{
		...
		{
			AssertWalletIsLoaded();
			var km = Global.GetDefaultWallet().WalletService.KeyManager;
			return new
			{
				walletFile = Global.GetDefaultWallet().WalletService.KeyManager.FilePath,
				...
				balance = Global.GetDefaultWallet().WalletService.Coins
		...
		public object GenerateReceiveAddress(string label)
		{
			...
			var hdkey = Global.GetDefaultWallet().WalletService.KeyManager
		...
		public string BuildTransaction(PaymentInfo[] payments, TxoRef[] coins, int feeTarget, string password = null)
		{
			...
			var result = Global.GetDefaultWallet().WalletService.BuildTransaction(
				password,
				payment,
				feeStrategy,
		...
		public async Task<object> SendTransactionAsync(PaymentInfo[] payments, TxoRef[] coins, int feeTarget, string password = null)
		{
			...
			var toDequeue = Global.GetDefaultWallet().WalletService.Coins
				.Where(x => x.CoinJoinInProgress && coins.Contains(x.GetTxoRef()))
				.ToArray();
			if (toDequeue.Any())
			{
				await Global.GetDefaultWallet().WalletService.ChaumianClient.DequeueCoinsFromMixAsync(toDequeue, DequeueReason.TransactionBuilding).ConfigureAwait(false);
			}
		...
		{
			AssertWalletIsLoaded();
			var txHistoryBuilder = new TransactionHistoryBuilder(Global.GetDefaultWallet().WalletService);
		...
		{
			AssertWalletIsLoaded();
			var keys = Global.GetDefaultWallet().WalletService.KeyManager.GetKeys(null);
		...
		public async Task EnqueueForCoinJoinAsync(TxoRef[] coins)
		{
			...
			AssertWalletIsLoaded();
			var coinsToMix = Global.GetDefaultWallet().WalletService.Coins.Where(x => coins.Any(y => y.TransactionId == x.TransactionId && y.Index == x.Index));
			await Global.GetDefaultWallet().WalletService.ChaumianClient.QueueCoinsToMixAsync(coinsToMix.ToArray()).ConfigureAwait(false);
		}
		...
		public async Task DequeueForCoinJoinAsync(TxoRef[] coins)
		{
			...
			AssertWalletIsLoaded();
			var coinsToDequeue = Global.GetDefaultWallet().WalletService.Coins.Where(x => coins.Any(y => y.TransactionId == x.TransactionId && y.Index == x.Index));
			await Global.GetDefaultWallet().WalletService.ChaumianClient.DequeueCoinsFromMixAsync(coinsToDequeue, DequeueReason.UserRequested).ConfigureAwait(false);
		}
		...
		{
			if (Global.GetDefaultWallet() is null)
			{
